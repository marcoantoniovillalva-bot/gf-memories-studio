...
Usuario dice algo
    |
    ├── "Nuevo cliente / nuovo progetto cliente / onboarding / raccogliere materiali"
    |       → Ejecutar skill CLIENT-INTAKE (raccolta materiali → CLIENT_BRIEF.md)
    |         SEMPRE prima di qualsiasi altro skill su un progetto cliente
    |
    ...
    |
    ├── "Crea una oferta / pitch / propuesta de valor irresistible"
    |       → Ejecutar skill ALEX-HORMOZI-PITCH (metodologia $100M Offers)
    |
    ├── "Disenha este componente / pagina / UI / interfaz"
    |       → Ejecutar skill FRONTEND-DESIGN (UI production-grade, anti AI slop)
    |
    ├── "Construye la marca / identidad visual / posicionamiento"
    |       → Ejecutar skill BRAND-BUILDING
    |
    ├── "Analiza competidores / quien rankea para / spy on competitors"
    |       → Ejecutar skill COMPETITOR-ANALYSIS
    |
    ├── "Aplica psicologia / sesgo cognitivo / por que compra la gente"
    |       → Ejecutar skill MARKETING-PSYCHOLOGY
    |
    ├── "Crea campana de ads / Google Ads / Meta Ads / paid media"
    |       → Ejecutar skill PAID-ADS
    |
    ├── "Auditoria SEO / por que no rankeo / problemas SEO"
    |       → Ejecutar skill SEO-AUDIT
    |
    ├── "Procesa video / genera audio / Vertex AI / Gemini multimodal"
    |       → Ejecutar skill VERTEX-MEDIA-MASTER
    |
    ├── "Edita video / corta / concatena / exporta / FFmpeg"
    |       → Ejecutar skill VIDEO-PROCESSING-EDITING
    |
    ├── No encaja en nada tecnico → usa SKILL-AUTO para routing automatico
    |
    └── No encaja en nada
...
## Skills: 28 Herramientas Especializadas

### Skill di Ingresso (SEMPRE PRIMA)

| # | Skill | Cuando usarlo |
|---|-------|---------------|
| 0 | `client-intake` | **PRIMO STEP** su ogni progetto cliente. Raccoglie logo, palette, social, chiavi API, recensioni → genera CLIENT_BRIEF.md |
...
### Marketing & Content (Villalva Stack)

| # | Skill | Cuando usarlo |
|---|-------|---------------|
| 19 | `alex-hormozi-pitch` | Crear oferta irresistible / pitch con metodologia $100M Offers |
| 20 | `frontend-design` | UI production-grade, anti "AI slop" — componentes y paginas con identidad visual fuerte |
| 21 | `brand-building` | Estrategia de marca, posicionamiento, voz y guia de identidad |
| 22 | `competitor-analysis` | Analisis competitivo SEO/GEO, keywords rivales, oportunidades de superar |
| 23 | `marketing-psychology` | 70+ modelos mentales y sesgos cognitivos aplicados al marketing |
| 24 | `paid-ads` | Campanas Google Ads / Meta Ads: copy, audiencias, ROAS, optimizacion |
| 25 | `seo-audit` | Auditoria tecnica SEO, on-page, meta tags, diagnostico de rankings |
| 26 | `vertex-media-master` | Google Vertex AI: video, audio, imagenes Imagen 4, campanas multimedia |
| 27 | `video-processing-editing` | FFmpeg: cortar, concatenar, exportar video para YouTube y social |
| 28 | `skill-auto` | Meta-router: detecta automaticamente el skill correcto para la tarea |
...
│
├── skills/                    # 28 skills especializados
│   ├── --- CORE FACTORY ---
...
│   ├── --- VILLALVA STACK (Marketing & Content) ---
│   ├── alex-hormozi-pitch/   # Ofertas irresistibles ($100M Offers)
│   ├── frontend-design/      # UI production-grade, anti AI slop
│   ├── brand-building/       # Estrategia de marca e identidad
│   ├── competitor-analysis/  # Analisis SEO/GEO competitivo
│   ├── marketing-psychology/ # 70+ modelos mentales para marketing
│   ├── paid-ads/             # Google Ads + Meta Ads
│   ├── seo-audit/            # Auditoria tecnica SEO
│   ├── vertex-media-master/  # Google Vertex AI: video, audio, imagen
│   ├── video-processing-editing/ # FFmpeg: edicion y exportacion video
│   └── skill-auto/           # Meta-router automatico de skills
│
...
## Proyectos Demo — Schema Aislado

Si `.env.local` contiene `NEXT_PUBLIC_SUPABASE_SCHEMA=demo_xxx`, el proyecto es una DEMO
que comparte el Supabase de Marketizzati con schema separado. Reglas obligatorias:

- **NUNCA** crear tablas en `public` — todo va en el schema demo
- Configurar el cliente Supabase con el schema del env:
...
- Todas las migraciones SQL deben empezar con `SET search_path TO demo_xxx, public;`
- Para limpiar: `DROP SCHEMA demo_xxx CASCADE;` — no toca nada de Marketizzati
